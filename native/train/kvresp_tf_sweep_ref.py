...
NSTP = 60; LR = 0.00005; B1 = 0.9; B2 = 0.999; EPP = 1e-8; WDD = 0.0
# PURE TEACHER-FORCED sweep (option ii diagnostic, EXTENDED 20->60): find where argmax-match
# over the 83 response labels saturates high enough for clean greedy autoregressive decode.
# 20-step result (2026-09-03): CE 15.96->2.289, argmax-match 0->72.29% and still accelerating
# (16%->72% over steps 14-20). Teacher-forced CE DOES yield an emitting adapter; the original
# 4-step commit was simply too short.
...
np.savetxt(os.path.join(out, "kvresp_tf_sweep_loss_ref.txt"), np.array(losses), fmt="%.10g")
