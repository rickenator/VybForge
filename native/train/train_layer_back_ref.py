...
np.savetxt(os.path.join(out, "b_dXN_ref.txt"), dXN.reshape(-1), fmt="%.10g")
np.savetxt(os.path.join(out, "b_dX1n_ref.txt"), (d_xn_g+d_xn_u).reshape(-1), fmt="%.10g")
...
np.savetxt(os.path.join(out, "b_dVatt_ref.txt"), gradv.reshape(-1), fmt="%.10g")   # attention V-grad (intermediate)
print("M2C_REF_DONE")
