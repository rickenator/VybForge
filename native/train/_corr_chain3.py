...
dLin=0.5*(xo-t)*2/(S*D)
dx=dLin.copy()
dxout={}   # dx_out(L) = gradient of layer-L input
for L in range(35,-1,-1):
    gu,dx=bwd(L,dx.copy(),cache[L])
    dxout[L]=dx.reshape(-1).copy()
for L in (35,17,0):
    gp=np.loadtxt(f"{out}/g_dg_L{L}.txt").reshape(-1)
    o=dxout[L]
    n=min(gp.size,o.size)
    print(f"L{L}: corr={np.corrcoef(gp[:n],o[:n])[0,1]:.6f} |gpu|={np.linalg.norm(gp):.4e} |ord|={np.linalg.norm(o):.4e}")
